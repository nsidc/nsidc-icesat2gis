...
def test__linestring_for_isolated_point(atl08_test_filepath):
    ...
    points_for_gt = points[points["beam"] == "gt3r"]
...
def test__read_points_for_gt_missing_raises_error(atl08_test_filepath):
    ...
    with pytest.raises(ICESat2MissingDataError):
        _read_points_for_gt(
            # We expect gt2l beam to be missing from the test data.
            beam="gt2l",
            ds=ds,
            filename=atl08_test_filepath.name,
            variables_to_include=ATL08_DEFAULT_GT_CORE_VARS,
...
def test__beam_strength_from_orientation():
    # Forward
    assert _beam_strength_from_orientation(beam="gt1l", orientation=1) == "weak"
    assert _beam_strength_from_orientation(beam="gt1r", orientation=1) == "strong"

    # Backward
    assert _beam_strength_from_orientation(beam="gt1l", orientation=0) == "strong"
    assert _beam_strength_from_orientation(beam="gt1r", orientation=0) == "weak"
